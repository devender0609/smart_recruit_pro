...
export default function Page() {
  ...
  function downloadCSV() {
    ...
    const rows = results.map((r: any, i:number) => [
      i+1, r.filename, ((r.score||0)*100).toFixed(1)+"%", (r.evidence||[]).join("; "),
      r.experience||"—", r.education||"—", r.charCount||0, r.notes||""
    ]);
      ...
      <section className="card p-6">
        ...
        <form onSubmit={onSubmit} className="grid gap-4">
          <label className="grid gap-2">
            ...
            <textarea className="w-full p-3 rounded-xl border focus:outline-none focus:ring-2 focus:ring-brand-300 min-h-[140px]"
              placeholder="Paste the job description here..." value={jd}
          ...
          <label className="grid gap-2">
            ...
            <input type="file" multiple accept=".pdf,.docx,.txt" onChange={(e)=>setFiles(e.target.files)} 
              className="file:mr-3 file:px-4 file:py-2 file:rounded-lg file:border-0 file:bg-brand-600 file:text-white file:hover:bg-brand-700 border rounded-xl p-2" required/>
          </label>

          <div className="flex gap-3 items-center flex-wrap">
            <button className="btn btn-primary" disabled={loading}>{loading?"Scoring...":"Generate Shortlist"}</button>
            <button type="button" className="btn btn-ghost" onClick={()=>{ setJd(""); setJdFile(null); setFiles(null); setResults([]); }}>Reset</button>
            <button type="button" className="btn btn-ghost" onClick={downloadCSV} disabled={!results.length}>Export CSV</button>
            <span className="badge border-brand-200 text-brand-700 bg-brand-50">OCR & embeddings optional via env</span>
          </div>
        </form>
      </section>
