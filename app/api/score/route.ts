...
export const runtime = "nodejs";         // <-- App Router way
...
function estimateYears(text: string) { const m = text.match(/(\d+)\s*(\+)?\s*(?:years|yrs)\b/i); return m ? m[0] : "—"; }
...
function snippet(text: string, jdTokens: string[]) {
  ...
  for (const k of jdTokens) {
    ...
    if (pos >= 0) { const start = Math.max(0,pos-80); const end = Math.min(text.length,pos+120); return text.slice(start,end)+(end<text.length?"...":""); }
  }
...
async function bufferToText(filename: string, buf: Buffer): Promise<string> {
  ...
  if (mime.includes("pdf") || filename.toLowerCase().endsWith(".pdf")) {
    try { const mod = await import("pdf-parse"); const pdf = (mod as any).default || (mod as any); const data = await pdf(buf); return data.text || ""; }
    catch { /* fall through */ }
  }
  if (mime.includes("officedocument.wordprocessingml.document") || filename.toLowerCase().endsWith(".docx")) {
    try { const mammoth = await import("mammoth"); const res = await (mammoth as any).extractRawText({ buffer: buf }); return res.value || ""; }
    catch { /* fall through */ }
  }
...
function scoreResume(jdText: string, resumeText: string, semBoost=0) {
  const jdTokensAll = tokenize(jdText), rTokensAll = tokenize(resumeText);
  const jdKeys = keywords(jdTokensAll, 3), rKeys = keywords(rTokensAll, 3);
  const jdSet = new Set(jdKeys), rSet = new Set(rKeys);
  ...
  return { score: finalScore, evidence: [...new Set([...overlap.slice(0,6), ...skillHits.slice(0,6)])].slice(0,8),
           experience: estimateYears(resumeText), education: detectEducation(resumeText), snippet: snippet(resumeText, jdKeys) };
}
